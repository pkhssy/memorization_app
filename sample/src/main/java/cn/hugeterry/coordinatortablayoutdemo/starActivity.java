...
public class starActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.star_contents);
        ...
        init();
        //Toast.makeText(getApplicationContext(),"Size: "+spelling.size(), Toast.LENGTH_SHORT).show();
        ...
        del.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(spelling.size() == 0){ // 내용물이 없을 경우 삭제가 안 되도록
                    ...
                    Toast.makeText(getApplicationContext(),"중요한 내용에서 삭제되었습니다.",Toast.LENGTH_SHORT).show();
    ...
    private void dataInit(int type){
        ...
        // 이미 DB에 있는 철자와 뜻 가져오기 (star = 1 일때 & type 고려)
        if(type ==1){ // 단어
            c = db.rawQuery("select spelling,meaning, category_name from contents where star = 1 and type = 1", null);
        }
        else if(type ==2) { // 문장
            c = db.rawQuery("select spelling,meaning, category_name from contents where star = 1 and type = 2", null);
        }
    ...
    private void setToolBar(){
        ...
        toolbar.setSubtitle("중요한 내용"); //부제목 넣기
    ...
    public void delete_contents() {
        ...
        String sql = "update contents set star = 0 where category_name='"+category.get(index)+ "' and spelling='"+spelling.get(index)+"';";
        db.execSQL(sql);
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        share.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem menuItem) {
                ...
                if(type ==1 ){
                    text = "[중요한 내용 - 문장/단어] : 단어";
                    c = db.rawQuery("select spelling,meaning, category_name from contents where star = 1 and type = 1", null);
                }
                else if (type == 2 ){
                    text = "[중요한 내용 - 문장/단어] : 문장";
                    c = db.rawQuery("select spelling,meaning, category_name from contents where star = 1 and type = 2", null);
                }
                while(c.moveToNext()){
